...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$states) {
        ...
        try {
            ...
            // 2) Insérer / mettre à jour les détails avec le champ remarque
            $sqlDetail = "
              INSERT INTO appel_detail (appel_id, eleve_id, statut, remarque, anneeScolaire)
              VALUES (?, ?, ?, ?, ?)
              ON DUPLICATE KEY UPDATE statut = VALUES(statut), remarque = VALUES(remarque)
            ";
            ...
            foreach ($states as $id => $etat) {
                ...
                $remarque = isset($remarques[$id]) ? trim((string)$remarques[$id]) : null;
                $annee    = $anneeEnCours;

                // Types : iisss -> appel_id (int), eleve_id (int), statut (string), remarque (string), anneeScolaire (string)
                $stmtDetail->bind_param('iisss', $appelId, $eleveId, $etat, $remarque, $annee);
...
// =====================
// Charger les statuts et remarques existants pour cette date
// =====================
$currentPresence = [];
$currentRemarques = [];
if ($appelId > 0) {
    $stmt = $con->prepare("SELECT eleve_id, statut, remarque FROM appel_detail WHERE appel_id = ?");
    if ($stmt) {
        ...
        foreach ($rs->fetch_all(MYSQLI_ASSOC) as $r) {
            $eId = (int)$r['eleve_id'];
            $currentPresence[$eId]  = $r['statut'];
            $currentRemarques[$eId] = $r['remarque'] ?? '';
        }
        $stmt->close();
    }
}
          ...
          <tr data-id="<?= $id ?>">
            ...
            <td>
              <input type="text" 
                     class="form-control form-control-sm" 
                     name="remarque[<?= $id ?>]" 
                     placeholder="Ex: Arrivée en retard, Justifié..." 
                     value="<?= e($remarque) ?>">
            </td>
          </tr>
        <?php endforeach; ?>
