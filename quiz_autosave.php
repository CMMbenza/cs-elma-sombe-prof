...
try {
    ...
    if ($quizId === 0) {
        ...
        $stmt = $con->prepare("
            INSERT INTO quiz
            (agent_id, cours_id, periode_id, type_quiz, format, titre, description, date_limite, statut, anneeScolaire)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'brouillon', ?)
        ");
...
if (!empty($classeIds)) {
    ...
    $stmtQC = $con->prepare("
        INSERT INTO quiz_classe (quiz_id, classe_id, anneeScolaire)
        VALUES (?, ?, ?)
    ");

    foreach ($classeIds as $cid) {
        $stmtQC->bind_param("iis", $quizId, $cid, $anneeEnCours);
        $stmtQC->execute();
    }
