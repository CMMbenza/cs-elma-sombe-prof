...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!hash_equals($csrf, $token)) {
    ...
    if ($titre === '' || $contenu === '') {
      ...
      $sqlIns = "INSERT INTO annonces (titre, contenu, sender_role, sender_id, dest_type, dest_id, anneeScolaire)
                 VALUES (?, ?, 'prof', ?, 'user', ?, ?)";
      if ($stmt = $con->prepare($sqlIns)) {
        $stmt->bind_param('ssiis', $titre, $contenu, $agentId, $directeurUserId, $anneeEnCours);
        if ($stmt->execute()) {
            ...
            <div class="card shadow-sm">
                <div class="card-body">
                    ...
                    <form method="post" class="needs-validation" novalidate>
                        ...
                        <div class="d-none small text-muted mt-3">
