...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($eleveId <= 0 || $trimestre === '' || !isset($_POST['lang']) || !isset($_POST['math']) || !isset($_POST['cult'])) {
        $err = "Veuillez remplir tous les champs obligatoires (Élève, Session et Notes).";
    } else {

        $lang = (float)$_POST['lang'];
        $math = (float)$_POST['math'];
        $cult = (float)$_POST['cult'];
...
/**
 * Retourne la place sous forme d'ordinal (ex: 1è, 2è, 3è, etc.)
 */
function getOrdinalSuffix(int $rank): string {
    return $rank . '<sup>è</sup>';
}
?>
...
<div class="container mt-3">
        ...
        <div class="card-body">
            ...
            <form method="post">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="eleve_id" class="form-select" required>
                            <option value="" disabled <?= !isset($editData['eleve_id']) ? 'selected' : '' ?>>
                                Sélectionner l'élève</option>
                            <?php foreach ($eleves as $el):
                                $nom = trim($el['nom'].' '.$el['postnom'].' '.$el['prenom']);
                                $selected = ((int)($editData['eleve_id'] ?? 0) === (int)$el['id']) ? 'selected' : '';
                            ?>
                    ...
                    <!-- TRIMESTRE -->
                    <div class="col-md-6">
                        <label class="form-label">Session scolaire (Période/Examen) <span
                                class="text-danger">*</span></label>
                        <select name="trimestre" class="form-select" required>
                            <option value="" disabled <?= !isset($editData['trimestre']) ? 'selected' : '' ?>>
                                Sélectionner la session scolaire (Période/Examen)</option>
                            <?php foreach ($trimestres as $t): ?>
                            <option value="<?= e($t) ?>" <?= ($editData['trimestre'] ?? '') === $t ? 'selected' : '' ?>>
                                <?= e($t) ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-2">
                        <label class="form-label">MAX. LANG <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" name="max_lang"
                            value="<?= e($editData['max_lang'] ?? 20) ?>" class="form-control"
                            <?= $editData ? 'readonly' : '' ?> required>
                    </div>

                    <div class="col-md-2">
                        <label class="form-label">MAX. MATH <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" name="max_math"
                            value="<?= e($editData['max_math'] ?? 20) ?>" class="form-control"
                            <?= $editData ? 'readonly' : '' ?> required>
                    </div>

                    <div class="col-md-2">
                        <label class="form-label">MAX. CULT <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" name="max_cult"
                            value="<?= e($editData['max_cult'] ?? 20) ?>" class="form-control"
                            <?= $editData ? 'readonly' : '' ?> required>
                    </div>
                ...
                <div class="row g-2">
                    ...
                    <div class="col-md-2">
                        <label class="form-label">LANG <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" name="lang" value="<?= e($editData['lang'] ?? '') ?>"
                            class="form-control" required>
                    </div>

                    <div class="col-md-2">
                        <label class="form-label">MATH <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" name="math" value="<?= e($editData['math'] ?? '') ?>"
                            class="form-control" required>
                    </div>

                    <div class="col-md-2">
                        <label class="form-label">CULT <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" name="cult" value="<?= e($editData['cult'] ?? '') ?>"
                            class="form-control" required>
                    </div>
                ...
                <div class="text-end mt-4">
                    ...
                    <a href="palmares_trimestre.php" class="btn btn-secondary">Annuler</a>
                    <?php else: ?>
                    <button type="submit" class="btn btn-primary">Valider la côte</button>
                    <?php endif; ?>
                </div>
    ...
    <!-- TABLE FILTRES -->
    <div class="card mb-3">
        <div class="card-header">Filtres & Export</div>
        <div class="card-body">
            <form method="get" class="row g-2">
                ...
                <div class="col-md-3">
                    <label class="form-label">Session scolaire</label>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <span>Historique des palmarès</span>
            <span class="badge bg-primary"><?= count($rows) ?> résultat(s)</span>
        </div>
            ...
            <table class="table table-bordered table-hover align-middle text-center">
                ...
                <tbody>
                    <tr class="table-warning fw-bold">
                        <td>#</td>
                        <td>—</td>
                        <td class="text-start">Pondération</td>
                        <td>—</td>
                        <td>/<?= e($rows[0]['max_lang'] ?? 0) ?></td>
                        <td>/<?= e($rows[0]['max_math'] ?? 0) ?></td>
                        <td>/<?= e($rows[0]['max_cult'] ?? 0) ?></td>
                        <td>/<?= e($rows[0]['max_total'] ?? 0) ?></td>
                        <td>/<?= e($rows[0]['max_percent'] ?? 100) ?></td>
                        <td colspan="2">Maxima</td>
                    </tr>

                    <?php if (!$rows): ?>
                    <tr>
                        <td colspan="11" class="text-muted py-4">Aucun résultat trouvé.</td>
                    </tr>
                    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <!-- PLACE / RANG DYNAMIQUE -->
                        <td class="fw-bold text-success">
                            <?= getOrdinalSuffix($rang) ?>
                        </td>

                        <!-- NOM -->
                        <td class="text-start"><?= e($nom) ?></td>

                        <!-- TRIMESTRE -->
                        <td>
                            <span class="badge bg-dark"><?= e($r['trimestre']) ?></span>
                        </td>
                        ...
                        <!-- TOTAL -->
                        <td class="fw-bold text-primary"><?= number_format((float)$r['total'], 2) ?></td>
...
function calculer() {
    ...
    let maxTotal = maxLang + maxMath + maxCult;
    let total = lang + math + cult;

    let percent = 0;
    if (maxTotal > 0) {
        percent = (total * maxPercent) / maxTotal;
    }

    document.getElementById('max_total').value = maxTotal.toFixed(2);
    document.getElementById('total').value = total.toFixed(2);
    document.getElementById('pourcentage').value = percent.toFixed(2);
}
